...
describe('list', () => {
  ...
  it('should handle a bunch of items', () => {
    expect(list(['1', '2', '3', '4', '5', '6', '7', '8'])).toBe('1, 2, 3, 4, 5, 6, 7 and 8');
  });
  it('should handle a bunch of items, some of which are null/undefined', () => {
    expect(list(['1', '2', '3', '4', null, '5', undefined, '6', '7', '8'])).toBe(
      '1, 2, 3, 4, 5, 6, 7 and 8',
    );
  });
  it('should handle no conjunction', () => {
    expect(list(['first', 'second', 'third'], 'none')).toBe('first, second, third');
  });
  it('should handle optional delimiter', () => {
    expect(list(['first', 'second', 'third'], 'none', 'semicolon')).toBe('first; second; third');
  });
  ...
  describe('serialComma option', () => {
    ...
    it('should use serial comma when serialComma is true for multiple items', () => {
      expect(list(['1', '2', '3', '4'], 'and', 'comma', { serialComma: true })).toBe(
        '1, 2, 3, and 4',
      );
    });
    ...
    it('should not affect two-item lists', () => {
      expect(list(['first', 'second'], 'and', 'comma', { serialComma: true })).toBe(
        'first and second',
      );
    });
    it('should not affect single-item lists', () => {
      expect(list(['first'], 'and', 'comma', { serialComma: true })).toBe('first');
    });
...
test('fbt.list()', () => {
  expect(
    fbt('Available Locations: ' + fbt.list('locations', ['Tokyo', 'London', 'Vienna']), 'Lists'),
  ).toMatchSnapshot();
});
...
test('<fbt:list>', () => {
  expect(
    <fbt desc="Lists">
      Available Locations: <fbt:list items={['Tokyo', 'London', 'Vienna']} name="locations" />.
    </fbt>,
  ).toMatchSnapshot();
});
