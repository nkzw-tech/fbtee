...
const parameterRegexp = new RegExp(String.raw`\{([^}]+)\}(` + PUNCT_CHAR_CLASS + '*)', 'g');
...
export default function substituteTokens(
  ...
  const stringPieces = template
    .replace(parameterRegexp, (_match: string, name: string, punctuation: string): string => {
  ...
  const pieces: Array<FbtContentItem> = stringPieces[0] !== '' ? [stringPieces[0]] : [];
