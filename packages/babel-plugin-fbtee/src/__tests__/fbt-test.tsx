...
describe('fbt() API: ', () => {
  describe('using FBT subject', () => {
    it('should accept "subject" as a parameter', () => {
      expect(
        snapshotTransform(withFbtImportStatement(`fbt("Foo", "Bar", {subject: foo});`)),
      ).toMatchSnapshot();
    });
  });
...
const describeFbtBindingTestCase = (requireStatement: string) => `${requireStatement};
      fbt("Foo", "Bar");`;

describe('fbt variable binding detection', () => {
  it(`should handle commonJS require()`, () => {
    expect(
      snapshotTransform(describeFbtBindingTestCase(`const fbt = require('fbtee')`)),
    ).toMatchSnapshot();
  });
